...
| Valley Metro developer discovery path | Official static/realtime discovery via the agency site | https://www.valleymetro.org/how-to-ride/tech-tools/developers | Environment proxy returned `HTTP/1.1 403 Forbidden` before origin connection. | The developer page and its linked feed/API materials could not be read. |
| City of Phoenix Open Data | Official City data-catalog discovery path linked from agency/developer materials when available | https://www.phoenixopendata.com/ | Environment proxy returned `HTTP/1.1 403 Forbidden` before origin connection. | The catalog search and any linked Valley Metro GTFS resource could not be inspected. |
...
The official discovery path for this pass is Valley Metro’s GTFS page, then the agency developer page, then the City of Phoenix Open Data catalog and the GTFS resource linked there. The first two agency pages and the City catalog were each attempted directly on 2026-07-24 UTC, but the environment proxy returned `HTTP/1.1 403 Forbidden` before origin connection. The GTFS landing-page proxy response had content type `text/plain` and content length `9`; curl reported transfer status `000`, zero origin payload bytes, and no origin response. No static archive, live vehicle-position/trip-update/alert endpoint, or key-bearing URL was verified, queried, or inferred from third-party directories. A maintainer must follow this official discovery path from an origin-capable environment before Pass 13.2.

## Official terms, licensing, and discovery conclusion

The official pages above are the authoritative discovery routes recorded for this pass. The access block prevented review of the Valley Metro developer page and its City of Phoenix Open Data links, so this pass cannot state that a specific City catalog dataset, feed URL, or API endpoint is current. It also cannot confirm a feed license, attribution wording, cache TTL, redistribution permission, rate limit, or whether a general API terms-of-use document governs GTFS static/realtime feeds. These are unresolved, not implicit permissions.

Before any relay or public replay capture, record the final agency and City dataset URLs, access date, license/terms version, required attribution, permitted redistribution of derived normalized JSON, allowed cache duration/polling rate, and whether an API key is required. If a general API terms page exists, obtain written or published confirmation that it applies to the identified GTFS feeds; otherwise treat feed-specific terms as controlling. Do not place a key-bearing URL, token, cookie, or credential in source, fixtures, browser code, or documentation.
