...
-- A complete date spine ensures LAG and rolling windows do not skip inactive months.
...
monthly_aggregates AS (
    SELECT
        ...
        COUNT(*) FILTER (WHERE metrics.Defect_Eligible)
            AS Defect_Eligible_Order_Count,
        COALESCE(
            SUM(metrics.Quantity) FILTER (WHERE metrics.Defect_Eligible),
            0
        ) AS Defect_Eligible_Units,
        COALESCE(
            SUM(metrics.Defective_Units) FILTER (WHERE metrics.Defect_Eligible),
            0
        ) AS Defective_Units,

        COUNT(*) FILTER (WHERE metrics.Compliance = 'Yes')
            AS Compliant_Order_Count

    FROM supplier_month_spine AS spine
    LEFT JOIN int_procurement_order_metrics AS metrics
