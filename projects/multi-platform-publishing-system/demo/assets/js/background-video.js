...
function normalizeItems(items = []) {
  return items
    .map((item) => {
      ...
      const src = item.src || item.playbackUrl || item.downloadUrl || (id ? `https://drive.google.com/uc?export=download&id=${encodeURIComponent(id)}` : "");
      const type = item.type || item.mimeType || guessVideoType(src);
      return { ...item, id, src, type, poster: item.poster || item.posterUrl || item.thumbnailUrl || (id ? `https://drive.google.com/thumbnail?id=${encodeURIComponent(id)}&sz=w1600` : "") };
    })
