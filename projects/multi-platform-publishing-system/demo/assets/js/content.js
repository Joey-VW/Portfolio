...
export function parseMediaItems(value) {
  return splitMediaCell(value).map((item) => {
    ...
    return { url, imageUrl: type === "image" ? url : "", thumbnailUrl: "", originalUrl: url, fileId: "", type };
  });
}
...
export function normalizePhoto(row, index = 0) {
  ...
  const originalUrl = extractUrl(getCell(row, ["photoUrl", "fileUrl", "fileLink", "url", "imageUrl"]));
