...
| 16. Repository architecture modernization | IN REVIEW | Passes 16.0-16.2 are merged. Pass 16.3 now adds the Vite multi-page `dist/` proof, build-output validator, and local route/data smoke coverage while keeping production on the repository-root deployment. Cloudflare preview approval, rendered browser parity, production `dist/` cutover, source relocation, and broader browser gates remain deferred or blocked. | Current `main`; no live credentials |
