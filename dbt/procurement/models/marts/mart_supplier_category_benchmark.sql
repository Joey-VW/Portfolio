...
WITH supplier_category_aggregates AS (
    SELECT
        ...
        COUNT(*) FILTER (WHERE Defect_Eligible) AS Defect_Eligible_Order_Count,
        COALESCE(SUM(Quantity) FILTER (WHERE Defect_Eligible), 0)
            AS Defect_Eligible_Units,
        COALESCE(SUM(Defective_Units) FILTER (WHERE Defect_Eligible), 0)
            AS Defective_Units,
        COUNT(*) FILTER (WHERE Compliance = 'Yes') AS Compliant_Order_Count
...
category_baselines AS (
    SELECT
        ...
        COUNT(*) FILTER (WHERE Delivered) AS Category_Delivered_Order_Count,
        COUNT(*) FILTER (WHERE On_Time) AS Category_On_Time_Order_Count,
        AVG(Lead_Days) FILTER (WHERE Delivered) AS Category_Avg_Lead_Days,
        ...
        COUNT(*) FILTER (WHERE Compliance = 'Yes') AS Category_Compliant_Order_Count,
        SUM(Savings_Value) / NULLIF(SUM(Gross_Value), 0) AS Category_Savings_Rate,
        CAST(COUNT(*) FILTER (WHERE On_Time) AS DOUBLE)
            / NULLIF(COUNT(*) FILTER (WHERE Delivered), 0) AS Category_On_Time_Rate,
        COALESCE(SUM(Defective_Units) FILTER (WHERE Defect_Eligible), 0)
            / NULLIF(COALESCE(SUM(Quantity) FILTER (WHERE Defect_Eligible), 0), 0)
            AS Category_Defect_Rate,
        CAST(COUNT(*) FILTER (WHERE Compliance = 'Yes') AS DOUBLE)
            / NULLIF(COUNT(*), 0) AS Category_Compliance_Rate
    FROM {{ ref('int_procurement_order_metrics') }}
...
SELECT
    ...
    Savings_Rate - Category_Savings_Rate AS Savings_Rate_vs_Category,
    Category_Delivered_Order_Count,
    Category_On_Time_Order_Count,
    Category_Avg_Lead_Days,
    Avg_Lead_Days - Category_Avg_Lead_Days AS Avg_Lead_Days_vs_Category,
    Category_On_Time_Rate,
    On_Time_Rate - Category_On_Time_Rate AS On_Time_Rate_vs_Category,
    Category_Defect_Eligible_Order_Count,
    Category_Defect_Eligible_Units,
    Category_Defective_Units,
    Category_Defect_Rate,
    Defect_Rate - Category_Defect_Rate AS Defect_Rate_vs_Category,
    Category_Compliant_Order_Count,
    Category_Compliance_Rate,
    Compliance_Rate - Category_Compliance_Rate AS Compliance_Rate_vs_Category,
